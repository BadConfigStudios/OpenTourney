...
Read together, the correct reconciliation is:

- A bye **does** count as a win in the numerator of the bye recipient's own win percentage (per §5.6.1's "counts as a win for that competitor's match record").
- A bye round is **excluded from the denominator** — "does not count as a round played when calculating that competitor's win percentage" (§5.6.1) — so a competitor who went 4–0 with one bye in a 5-round event has win% = 4 / 4 = 100%, not 4 / 5 = 80%. The "total number of rounds in the tournament" language in §5.3.3.1 must be read as "total rounds the competitor actually played," consistent with §5.6.1, not the raw round count of the event.
- This adjusted win% (bye counted in numerator, bye round excluded from denominator) is the figure that then gets floored at 25% / capped at 100% or 75%, and is the value averaged into whichever *other* competitors list this competitor as an opponent for Op Win% purposes.
- The "Please Note" in §5.3.3.1 — "rounds in which a competitor received a random bye do not count as a win for that competitor" — read in isolation could be misinterpreted as "byes don't count as wins at all." That is not the correct reading; §5.6.1 is unambiguous that a bye **is** a win for match-record purposes. The "Please Note" is best read as reinforcing that the bye round itself is excluded from the *win-percentage calculation* (i.e., not treated as a played round that contributes a win/round pair), not that the win itself is discarded from the competitor's record.

**Why this differs from FR25/MTG's bye handling:** FR25's `OwpOomwTiebreak` (per `tcg-ruleset-research.md` §1) excludes the bye "opponent" from the *averaging step* — a player who received a bye simply isn't counted as one of their opponents' opponents, and the bye's effect on the bye-recipient's own MWP is not separately adjusted (MWP is computed as match points / (3 × rounds played) with the bye round included as a played round worth full match points). Pokémon's mechanism instead adjusts the **bye recipient's own win% denominator directly** — removing the bye round from "rounds played" before computing that competitor's own win%, which is a different place in the pipeline than where MTG's exclusion happens. A ruleset implementation that reuses FR25's "exclude bye from the average" logic verbatim for Pokémon would produce the wrong number: Pokémon needs the bye excluded from the *bye recipient's own denominator*, independent of who is averaging whom.

**Source:** Play! Pokémon Tournament Rules Handbook §5.3.3.1 (Please Note), §5.6.1.
...
1. **Match-record effect:** bye = win, for match points and overall record purposes.
2. **Win%-denominator effect:** bye round excluded from "rounds played" when computing that competitor's own win% (the mechanism discussed in §3 above).
...
3. **A different bye mechanism:** MTG excludes the bye from the *averaging* step; Pokémon adjusts the bye recipient's *own win% denominator*. These are not interchangeable — implementing Pokémon's bye rule by copying MTG's "exclude bye from average" logic would compute the wrong Op Win% for anyone who faced a competitor who received a bye earlier in the event.

Additionally, Pokémon needs a **head-to-head pairwise fallback** (§6) that isn't part of Family A's chain as documented for MTG/Lorcana. This is the concern FR29 scopes out as extending `TiebreakStrategy` with a pairwise comparator, separate from the scalar Op Win%/Op Op Win% steps that FR28 covers. Recommendation: model FR28 as a Family-A-derived strategy with its own config object (floor=0.25, cap={completed:1.0, dropped:0.75}, bye-denominator-adjustment=true, chain-depth=2 i.e. Op Win% + Op Op Win% only, no game-level step), and model FR29 as a separate fallback strategy consumed by FR28's strategy when the percentage chain terminates in an unresolved tie, rather than trying to force head-to-head into the same scalar-averaging code path.

---

## 9. Open verification items

- [ ] Confirm whether "the total number of rounds in the tournament" in §5.3.3.1's completed-competitor case is meant literally (raw scheduled round count) or should already be understood as "rounds played" given §5.6.1's bye-exclusion language — this doc takes the position they reconcile to the latter (see §3), but the handbook's own wording is not fully self-consistent on this point and should be double-checked against Play! Pokémon's official rulings/FAQ or a support ticket if available, before hardcoding the reconciliation as the sole interpretation.
